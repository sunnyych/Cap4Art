...
EMOTION2IDX = {emotion: idx for idx, emotion in enumerate(TYPE_LIST)}

# class_counts = training['TYPE'].value_counts().reindex(TYPE_LIST).fillna(0).values
# total_samples = class_counts.sum()
# class_weights = total_samples / (len(TYPE_LIST) * class_counts)
...
trained_model_dir = "./trained_semart_type"
...
for img_path in tqdm(test_image_paths, desc="Classifying test images"):
    ...
    with torch.no_grad():
        ...
        pred_label = TYPE_LIST[pred_class_idx]
...
df_preds.to_csv("semart_type.csv", index=False)
