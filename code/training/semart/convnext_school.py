...
SCHOOL_LIST = ["Italian", "Dutch", "French", "Flemish", "German", "Spanish", "English", "Netherlandish", "Austrian", 
                "Hungarian", "American", "Danish", "Swiss", "Russian", "Scottish", "Belgian", "Greek",
                "Catalan", "Bohemian", "Swedish", "Other", "Irish", "Norwegian", "Polish", "Finnish", "Portuguese"]

EMOTION2IDX = {emotion: idx for idx, emotion in enumerate(SCHOOL_LIST)}

# class_counts = training['SCHOOL'].value_counts().reindex(SCHOOL_LIST).fillna(0).values
# total_samples = class_counts.sum()
# class_weights = total_samples / (len(SCHOOL_LIST) * class_counts)
...
trained_model_dir = "./trained_semart_school"
...
for img_path in tqdm(test_image_paths, desc="Classifying test images"):
    ...
    with torch.no_grad():
        ...
        pred_label = SCHOOL_LIST[pred_class_idx]
...
df_preds.to_csv("semart_school.csv", index=False)
