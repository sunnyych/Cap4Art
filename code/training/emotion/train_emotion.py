...
# ===============================
# Load the dataset
df_artemis_processed = pd.read_csv("wikiart_training.csv")
...
EMOTION_LIST = [
    'agreeableness', 'anger',
       'anticipation', 'arrogance', 'disagreeableness', 'disgust', 'fear',
...
class PaintingEmotionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        label = EMOTION2IDX[row['majority_emotion']]
...
df_temp, df_test = train_test_split(
    ...
    test_size=0.1,
    random_state=42,
    stratify=df_artemis_processed['majority_emotion']
)
...
df_train, df_val = train_test_split(
    ...
    random_state=42,
    stratify=df_temp['majority_emotion']
)
...
# # Compute weights
# label_counts = Counter(df_train['majority_emotion'])
# total = sum(label_counts.values())
# class_weights = [total / label_counts[emotion] for emotion in EMOTION_LIST]
...
trained_model_dir = "./trained_wikiart"
os.makedirs(trained_model_dir, exist_ok=True)

# Save full model safely
model_path = os.path.join(trained_model_dir, "vit_emotion_model.pt")
...
df_preds.to_csv("wikiart_results.csv", index=False)
